...
int Graph::kruskalMST() { 
    // archivo csv
    fstream ReadFile("Datos_Grafo_K_3T.csv",ios::in);
    if(!ReadFile){
        cout<<"Creando archivo\n";
        fstream CreateFile("Datos_Grafo_K_3T.csv",ios::out);
    ...
    My_File.open("Datos_Grafo_K_3T.csv",ios::app);
    My_File.close();
} 
...
int main(){ 
    ...
    fe.open("E:\\discretasss\\Archivo_Vertices_Aristas_3T.txt");
    int a,b,c;
    for (int i=0; i<102;i++){
        int p=0,s=0,t=0;
        for(int j=0; j<3;j++){
